...
- Write workflow scripts only in the supported Elisp subset
- The workflow_run source must be raw Elisp text, not Markdown; do not wrap it in Markdown code fences
- Before calling workflow_run, mentally validate that every opening parenthesis, double quote, and quoted list is closed
...
Workflow DSL forms:
- (workflow "name" body...) defines one run; use one top-level workflow form
- (concurrency n) sets the max concurrently running worker agents; default is 5
- (phase "name" body...) groups tasks and records status
- (parallel expr...) evaluates branches concurrently; use for independent agents inside a phase
- (series expr...) evaluates branches sequentially when ordering is important
- (agent "name" :prompt "..." [:mode "plan|agent|yolo"] [:work-dir "..."] [:tools '("read" "grep")] [:max-iterations n] [:system-prompt-extra "..."]) runs one worker
...
- Always produce one complete top-level form: (workflow "name" ...)
- Every workflow form must be parenthesized; never write bare prose such as workflow "name" or phase "scan"
...
- Give every worker a complete bounded prompt with paths, expected output, and stop conditions
- Use explicit phase and agent names because result keys are "phase.agent"
- Pass prior phase outputs with (result ...) or (results ...), often via concat
- Keep dynamic workflow state in worker prompts and workflow store; do not ask to mutate system prompts or tool definitions at runtime
- If workflow_run returns a parse or arity error, fix the same Elisp source and retry once before changing the plan
  ...
  (phase "scan"
    (parallel
      (agent "worker-a"
        :mode "plan"
        :tools '("read" "grep")
        :prompt "Bounded task prompt with paths, expected output, and stop condition.")
      (agent "worker-b"
        :mode "plan"
        :tools '("read" "grep")
        :prompt "Another bounded task prompt.")))
  (phase "verify"
    (agent "cross-check"
      :mode "plan"
      :tools '("read" "grep")
      :prompt (concat (results "scan") "\nVerify the findings and return final risks."))))

Syntax checklist before workflow_run:
- Source starts with "(workflow " and ends with the matching ")"
- Parentheses are balanced; each phase, parallel/series, and agent form has a closing ")"
- Every string starts and ends with a double quote; embedded newlines are written as \n or split with concat
- Every agent option is a keyword/value pair; :prompt is present for every agent
  ...
  (phase "scan"
    (parallel
      (agent "gateway"
        ...
        :prompt "Audit internal/gateway authentication risks. Return file:line evidence and concrete risks.")
      (agent "hermes"
        :mode "plan"
        :tools '("read" "grep" "find")
        :prompt "Audit internal/hermes authentication risks. Return file:line evidence and concrete risks.")))
  (phase "verify"
    (agent "cross-check"
      :mode "plan"
      :tools '("read" "grep")
      :prompt (concat (results "scan") "\nReconcile contradictions, verify evidence, and list final risks."))))
`)
	}
