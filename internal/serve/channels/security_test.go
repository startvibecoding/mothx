...
func TestChannelSessionDirEncodesUnsafeComponents(t *testing.T) {
	root := t.TempDir()
	d := &Dispatcher{sessionDir: root}

	dir := d.channelSessionDir("wechat", "../evil/user")
	rel, err := filepath.Rel(filepath.Join(root, "channels"), dir)
	if err != nil {
...
func TestShouldAutoApprove(t *testing.T) {
	...
	cfg2 := &Config{Security: SecurityConfig{SmartApprovals: false}}
	sec2 := NewSecurity(cfg2)
