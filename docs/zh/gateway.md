...
Gateway 模式将 MothX 作为 HTTP 服务运行，对外暴露**标准 OpenAI Chat Completions API**。任何兼容 OpenAI 的客户端 — Cursor、Continue、Open WebUI、Python SDK、自定义脚本 — 都可以直接接入，MothX agent 在后台透明地执行工具调用。
...
3. `~/.vibecoding/gateway.json`（全局）

生成配置模板：

```bash
mothx --init-gateway
mothx --init-gateway --force  # 强制覆盖
```
