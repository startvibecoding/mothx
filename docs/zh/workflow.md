...
Gateway 和 ACP 也支持 workflow 模式，在对应配置中开启即可。
...
| `:mode` | string | 运行模式：`plan` / `agent` / `yolo`，默认继承父 agent |
| `:tools` | string list | 可用工具列表，用 `'("read" "grep")` 语法 |
| `:work-dir` | string | 工作目录，默认继承父 agent |
| `:max-iterations` | number | 最大迭代次数，默认 50 |
...
CLI 和 Gateway 中都可以用 `/workflows` 命令查看和管理 workflow 运行：
...
`workflow_run` 支持可选的 `timeoutSeconds` 参数。省略时使用默认工具超时；有明确上限的长任务可以设置正数秒数；只有确实需要持续运行的 workflow 才设置为 `0`，表示不加 agent 级 deadline。Gateway 模式下，`requestTimeoutSeconds` 仍然是外层 HTTP 请求 deadline，长时间请求也需要同步调大这个配置。
