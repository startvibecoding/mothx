...
| 结果不回写 | `/btw` 的问答只在浮动层展示，结束后不进入主 session，不进入 JSONL 持久化 |
...
- sub-agent 的 message history、工具调用、最终回答全部**不**写回主 agent，也**不**写入主 session JSONL。
...
- **不回写**：旁路问答不进入主 session JSONL，不污染审计轨迹（如需审计，可单独记录到 `btw.log`，待评审）。
