...
│   └── 20260529-120000_abc12345.db
│
└── hermes/                             # Hermes sessions（新增）
    ├── wechat/                         # 按平台分
    │   ├── wxid_user1/                 # 按用户分
    │   │   └── active.db            # 当前活跃 session
    │   └── wxid_user2/
    │       └── active.db
    ├── feishu/
    │   └── ou_user1/
...
**命名规则**：
- `active.db` — 当前活跃 session，每个用户始终只有一个
- `/new` 时：`active.db` → 重命名为 `<timestamp>_<id>.db`（归档），然后创建新的 `active.db`
...
├── active.db                        # 新 session
└── 20260529-120000_abc12345.db       # 归档的旧 session
```
...
用户首次发消息
  │
  ├─ 检查 hermes/<platform>/<user_id>/active.db
  │   ├─ 存在 → 加载并继续对话
  │   └─ 不存在 → 创建新 active.db（cwd = 平台配置的 work_dir）
  │
  ├─ 持续对话… (消息追加到 active.db)
  │
  ├─ context 接近上限 → 自动 compaction（不新建 session）
  │
  ├─ 用户发送 /new
  │   ├─ active.db 重命名为 <timestamp>_<id>.db
  │   └─ 创建新的 active.db
  │
  └─ 用户发送 /sessions
      └─ 列出当前 + 历史 sessions
...
Hermes 完全复用现有的 `session.Manager` 进行 SQLite 读写，只在上层包装路由逻辑：
...
func (d *Dispatcher) rotateSession(platform, userID string) (*session.Manager, error) {
    ...
    // 归档: active.db → <timestamp>_<id>.db
    if mgr, err := session.Open(activePath); err == nil {
        hdr := mgr.GetHeader()
        archived := filepath.Join(dir, fmt.Sprintf("%s_%s.db",
