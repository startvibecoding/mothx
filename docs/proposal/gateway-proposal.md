...
Gateway 模式将 MothX 作为一个 HTTP 服务启动，对外暴露**标准 OpenAI Chat Completions API** (`/v1/chat/completions`)。
任何兼容 OpenAI SDK 的客户端（Cursor、Continue、Open WebUI、自定义脚本等）都可以直接接入，
后端实际由 MothX agent 完成推理 + tool use 循环，对调用方完全透明。
...
| **独立配置文件** | `gateway.json`，与 `settings.json` 同目录 (`~/.vibecoding/`) |
...
# 指定端口
mothx gateway --port 9090

# 指定 provider/model（覆盖 settings.json 默认值）
mothx gateway --provider deepseek-openai --model deepseek-v4-flash

# 指定默认工作目录
mothx gateway --work-dir /home/user/projects

# 指定配置文件路径
mothx gateway --config /path/to/gateway.json

# 启用 sub-agent
mothx gateway --multi-agent

# 启用 sandbox
mothx gateway --sandbox

# 启用 debug
mothx gateway --debug --verbose
```

### 初始化配置文件

```bash
# 创建 gateway.json 模板（写入 ~/.vibecoding/gateway.json）
mothx --init-gateway

# 如果文件已存在，不覆盖，提示用户
mothx --init-gateway
# → gateway.json already exists: ~/.vibecoding/gateway.json
...
`gateway.json` 位于 `config.ConfigDir()` （通常 `~/.vibecoding/gateway.json`），与 `settings.json` 同目录。
...
```
POST   /v1/mothx/sessions          创建 session
GET    /v1/mothx/sessions           列出 session
GET    /v1/mothx/sessions/:id       获取 session 详情
DELETE /v1/mothx/sessions/:id       删除 session
```

这些是扩展端点，非 OpenAI 标准，前缀 `/v1/mothx/` 以区分。
...
4. ChatHandler
    ...
    │
    ├─ 4d. 将 OpenAI messages 转换为 MothX 内部格式
...
| `internal/session` | SQLite session 存储（每个 gateway session 一个 Manager） |
...
### 8.1 输入转换 (OpenAI → MothX)

```
OpenAI messages[] ──►  MothX 内部
─────────────────      ──────────────────
...
| `"ignore"` | 忽略客户端 system message。完全使用 MothX 内置 system prompt，适合不希望客户端干扰 agent 行为的场景。 |

**其他关键决策**:
- 只取最后一条 `user` 消息作为 `agent.Run(ctx, userMsg)` 的输入
- 之前的历史消息通过 `agent.LoadHistoryMessages()` 注入

### 8.2 输出转换 (MothX Event → OpenAI)

```
MothX Event         OpenAI Chunk (toolVisibility 决定)
──────────────           ───────────────
...
11. **`cmd/mothx/main.go`** — 添加 `gateway` 子命令 + `--init-gateway` flag
...
使用 `net/http` 标准库。MothX 定位轻量，不需要 gin/echo/fiber。中间件用 `http.Handler` 包装即可。
...
- Session 不持久化跨重启（重启清空），但 SQLite 数据库保留可恢复
...
完全忽略会让用户困惑。追加模式既保留了 MothX 的完整能力，又尊重客户端的自定义指令。

### D10: --init-gateway 配置初始化

`mothx --init-gateway` 生成 `gateway.json` 模板到 `~/.vibecoding/gateway.json`。
