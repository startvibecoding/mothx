...
- TUI 和 Serve 都已有 slash command 框架。
...
Elisp VM 不在 `mothx` 仓库内实现。`vibeEmacsLispVm` 已作为独立 Go module 发布到 GitHub，
提供通用 Elisp subset parser/evaluator 和 Go 侧函数注册 API。`mothx` 通过 `go.mod`
...
```bash
mothx --workflows
mothx --workflows --mode agent
mothx serve --workflows
mothx acp --workflows
```
...
```text
用 workflow 并行审计 internal/serve/openaiapi 和 internal/serve/channels 的认证风险，最后交叉验证结论。
```
  ...
  (phase "scan"
    (parallel
      (agent "api"
        ...
        :prompt "Audit internal/serve/openaiapi for authentication and workdir validation risks. Return findings with file paths and line numbers.")
      (agent "channels"
        :mode "plan"
        :tools '("read" "grep" "find")
        :prompt "Audit internal/serve/channels for authentication and webhook security risks. Return findings with file paths and line numbers.")))
  (phase "verify"
    (agent "cross-check"
      :mode "plan"
      :tools '("read" "grep" "find")
...
Serve 也提供同名 slash commands，便于 HTTP 客户端查询 workflow 状态。
...
3. `vibeEmacsLispVm` evaluator 执行 AST，并通过 `mothx` 注册进去的 workflow builtins 调用 `WorkflowHost`。
...
- `cmd/mothx/main.go`: 已增加 `--workflows` 启动参数；启用时注册 workflow tools。
...
- 已补充 CLI flag、AgentFactory worker prompt 隔离、workflow tool 注册隔离和 Serve help 的回归测试。
...
3. [x] 发布 `vibeEmacsLispVm` 到 GitHub，并在 `mothx` 通过 `go.mod` 引入 `v0.0.1`。
4. [x] 新增 `internal/workflow/lisp.go` 最小接入层和依赖验证测试。
5. [x] 在 `mothx` 中实现 `WorkflowHost` 接口和 fake host 测试，验证 `parallel`、`series`、`result`。
6. [x] 接入真实 `AgentManager`，实现 `workflow_run` / `workflow_status` / `workflow_cancel` tools。
7. [x] 增加 `--workflows` 启动参数，在 CLI / ACP / Serve 路径独立注册 workflow tools。
8. [x] 增加 TUI `/workflows` 和 Serve `/workflows` 查询入口。
...
- [x] `mothx` 不内嵌 Elisp parser/evaluator；已通过 `go.mod` 依赖该库。
...
- 决策更新: Elisp VM 在 `/home/free/src/vibeEmacsLispVm` 作为独立 Go library 开发，不放进 `mothx` 仓库。发布到 GitHub 后，`mothx` 已把它作为第三方依赖引入。
...
- 实施记录: `vibeEmacsLispVm` 已发布到 GitHub，`mothx` 已通过 `go.mod` 引入 `github.com/startvibecoding/vibeEmacsLispVm v0.0.1`，并新增 `internal/workflow` 的最小 VM 接入测试。
- 实施记录: `mothx` 已新增 workflow runner/store/tools/AgentHost，接入 `--workflows` CLI/ACP/Serve 开关，补充 TUI/Serve `/workflows` 状态查询命令。
...
- 实施记录: 已实现进程内 active workflow registry。`workflow_run` 运行时注册 cancel func，完成后自动移除；`workflow_cancel` 和 TUI/Serve `/workflows cancel <id>` 可取消当前进程内 active run，并将 run state 保存为 `canceled`。
