...
> 日期: 2026-07-09
> 目标版本: 待定
> 最近更新: Web UI 已支持 session 级工具勾选；聊天历史和实时输出已统一为 transcript 渲染；run/capability event 已有独立表、读取 API 和 Web UI 展示
...
7. `serve.json` 中是否应该保留 `workDir`，还是改名为 `defaultWorkDir` 来减少误解？
