...
> 日期: 2026-07-09
> 目标版本: 待定
> 最近更新: Web UI 已支持 session 级工具勾选；聊天历史和实时输出已统一为 transcript 渲染；run/capability event 已有独立表、读取 API 和 Web UI 展示；运行中 session 支持刷新/切换后的私有 SSE 续流
...
| workDir 属于 session | serve config 只提供 `defaultWorkDir` API fallback，不应覆盖 Web UI 或已存在 session 的 cwd |
...
| 安全边界独立于默认路径 | `allowedWorkDirs`、sandbox、auth token 控制权限；`defaultWorkDir` 只是直接 API 调用的 fallback |

## 4. WorkDir / Session 语义

### 4.1 配置层

`serve.json` 中的 `defaultWorkDir` 应理解为：

- 直接访问 OpenAI-compatible API 时，请求未显式提供 `x_working_dir` 且没有 session 时的 fallback。
- Web UI 目录选择器的初始打开位置之一，但不代表 Web UI session 的最终工作目录。
- 旧配置字段 `workDir` 只作为读取兼容别名，保存时应输出为 `defaultWorkDir`。
...
- Web UI 新 session 可以绕过用户明确选择工作目录。

`allowedWorkDirs` 应理解为 Web UI 可选择工作目录和 `x_working_dir` override 的安全边界：

- 未配置时不限制目录覆盖；Web UI 可以从 `defaultWorkDir` 打开目录选择器，并继续浏览其它本机目录。
- 配置为一个或多个目录时，只允许选择这些目录及其子目录。
- 配置为空数组 `[]` 时，禁止工作目录覆盖。
...
1. Web UI 新建 session 时必须明确选择 workDir；直接 API 请求可以省略并使用 `defaultWorkDir` fallback。
...
`GET /api/sessions` 默认返回 `sessions.db` 中的全部 session，并合并 active runtime 状态。这个默认面向本机工作台场景；远程或多用户 serve 可以在后续通过单独的安全模式增加 owner/namespace/policy 过滤，但不改变本机默认全量浏览语义。
...
- `/v1/chat/completions` 支持 VibeCoding 扩展字段 `x_transcript`；Web UI 请求开启后，实时 SSE 会发送 `event: transcript`，其消息形态复用历史接口的 `SessionMessageEntry`，使实时输出和历史聊天记录使用同一套渲染结构。
...
- `sessions.db.session_run_events` 和 `sessions.db.session_capability_events` 已作为独立事件表落地；chat run 会写 started/finished/failed/canceled 事件，capability PATCH、`x_tools`、`/mode`、`/delegate` 会在状态实际变化时写 capability change event。
- `GET /api/sessions/{id}/run-events` 和 `GET /api/sessions/{id}/capability-events` 已提供结构化事件读取；Web UI 会在聊天记录下方展示最近 run/capability event。
- `GET /api/sessions/{id}/stream` 已作为 Web UI 私有 SSE 续流接口落地：连接时先按 `seq` cursor 从 `sessions.db` 补齐消息、run event 和 capability event，运行中再订阅当前进程的 in-memory fanout，输出 `transcript`、`run_event`、`capability_event`、`done` 等事件。
...
`x_tools` 的语义是“把当前 Web UI 中该 session 的工具选择同步到本次 session runtime”。它适合作为 Web UI 的 chat-run 同步层，但不替代正式 capability API：读取、PATCH 和当前状态持久化已经由 session capability API 与 `session_capabilities` 承担；审计读取由独立 run/capability event 表和事件 API 承担。
...
GET   /api/sessions/{id}/capability-events
...
- `sessions.db.session_capabilities` 保存 session 当前能力状态，只表达“现在是什么”。
- `sessions.db.session_capability_events` 用于追加 capability change event，只表达“为什么变成这样”；它记录旧值、新值、来源、actor、runID、时间和原因，不写入 message 表，也不混入 `session_capabilities` 当前状态表。
- `sessions.db.session_run_events` 用于追加 run lifecycle event，只表达“一次运行发生了什么”；它记录 run started/finished/canceled/failed、request snapshot、capability snapshot、usage、error、tool/workflow/sub-agent 关联等，不写入 message 表。
...
- `x_transcript` 是 Web UI 私有但稳定的扩展字段；请求为 `true` 时，SSE 除 OpenAI-compatible `data:` chunk 外，还发送 `event: transcript`，事件 payload 复用历史消息的 `SessionMessageEntry` 形态。Web UI 使用 transcript 事件渲染实时消息；普通 OpenAI 客户端可以忽略该事件。
- run event 写入独立 `session_run_events` 表，用于刷新恢复、running 状态、cancel/retry、usage/error 审计和 Agent Board 关联；不把 run lifecycle 写入 message 表。
- `GET /api/sessions/{id}/stream` 是 Web UI 私有续流接口，不替代 `/v1/chat/completions`。它面向刷新页面、切换 session 后原 chat SSE 已断开的场景：先基于 `sessions.db` cursor 做一次增量回放，再接当前进程内的 active session fanout，结束时发送 `done` 并由 Web UI 重载 canonical history。
...
- 已完成事件存储版：独立 `session_capability_events` 表和 `session_run_events` 表已落地，并写入 run lifecycle 与 capability change event。
- 已完成审计展示版：`GET /api/sessions/{id}/run-events`、`GET /api/sessions/{id}/capability-events` 和 Web UI 最近事件展示已落地。
- 已完成运行中续流版：`GET /api/sessions/{id}/stream` 支持 DB cursor 回放 + in-memory fanout，Web UI 切换/刷新到 running session 后可继续看到新增输出和事件。
- 待完成运行策略版：补运行中 request 的切换策略和冲突提示。
...
- 不让 serve config 的 `defaultWorkDir` 成为全局 session 过滤器。
...
3. 多用户 serve 场景下，session 列表是否需要 owner / namespace？
4. channels 创建的 per-user session 是否和普通 API/TUI session 在同一列表展示？
5. sub-agent 是否应该拥有独立可见 session，还是只作为父 session 的运行事件？
6. workflow worker 的结果是否需要持久化为独立消息、运行记录，还是只保存在 workflow store？
7. 已决定：`serve.json` 保存时使用 `defaultWorkDir`，旧 `workDir` 仅保留读取兼容。
8. Web UI 是否应该提供 `/api/sessions/active`，把 active runtime 与历史 session 明确分层？
9. session title 应该来自首条用户消息、显式重命名，还是由模型摘要生成？
10. mode/delegate/multi-agent/workflow/webSearch/browser/a2aMaster 的当前状态存在 `session_capabilities`；是否还需要从 capability events 回放校验当前状态？
11. multi-agent、delegate、webSearch、browser、a2aMaster 是 session 级开关，还是 serve 级全局开关加 session opt-in？
12. Web UI 切换能力时，是否允许用 slash command 作为兼容层，还是必须走结构化 API？
13. 运行中 request 存在时，切换 mode 或工具集应该禁止、排队，还是自动取消当前 run？
14. `yolo`、sandbox off、外部 channel 自动执行等高风险能力的 policy 边界放在 `serve.json` 还是独立 security 配置？
15. Settings UI 的 Advanced JSON 是否允许直接保存，还是只能导出/导入并经过 validate？
16. 配置写入时如何展示 global/project/session 三层覆盖关系？
17. 哪些配置变更需要重建 session runtime，哪些需要重启 serve？
...
4. Web UI 新建 session 继续要求用户选择 workDir；serve config 的 `defaultWorkDir` 只服务直接 API fallback。
...
9. `x_transcript` 固化为 Web UI 私有稳定 SSE 扩展；历史和实时聊天都以 `SessionMessageEntry` 形态渲染。
10. run event 和 capability event 都使用独立新表：`session_run_events`、`session_capability_events`；它们只做追加记录，不污染 message 表、session 表或 `session_capabilities` 当前状态表。
11. Settings 先做结构化 UI，不把 JSON viewer 当作正式配置界面。
12. Settings 写入必须使用按域 patch 和 validate，避免覆盖用户未修改字段。
