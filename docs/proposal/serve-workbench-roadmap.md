...
- **核心能力开关中心**：mode、delegate、multi-agent、workflow、thinking、sandbox 等能力都应能在 serve 中查看和切换。
...
| 能力开关显式化 | CLI flag 和 slash command 背后的核心能力要在 Web UI/API 中有结构化状态 |
...
| Channels/Cron/A2A | serve 级能力，和 session 运行态关联展示 | Agent Board / Settings |

### 6.3 切换语义

切换 mode、delegate、multi-agent、workflow 这类能力会改变 system prompt、tool registry 或执行策略，因此不能只是改 UI 状态。建议规则：
...
- `sessions.db` 保存 session 当前能力状态，或保存一组 capability change event 并计算当前状态。
- active runtime 只保存内存中的 agent 实例、运行中 request 和 tool/workflow 状态。

这样做的好处是：重启 serve 后，历史 session 的 mode/delegate/multi-agent/workflow 状态仍可恢复；同时 serve 的启动参数只影响新 session，不会意外改写已有 session。
...
- 顶部提供 mode/model/thinking/delegate/multi-agent/workflow 等能力开关。
...
| Capabilities | delegate、multi-agent、workflow、channels、cron、A2A | 开关、状态、依赖检查 |
...
- 明确 mode/delegate/multi-agent/workflow 是 session 能力开关，而不是只属于启动参数。
...
- 新增 capability model，覆盖 mode、model、thinking、delegate、multi-agent、workflow、sandbox 状态。
- Web UI 在 Chat 顶部展示可切换能力。
...
11. mode/delegate/multi-agent/workflow 的当前值是存在 session metadata，还是通过事件流计算？
12. multi-agent 和 delegate 是 session 级开关，还是 serve 级全局开关加 session opt-in？
...
2. active pool 只用于补充 `active`、`mode`、`delegateMode`、`workflows` 等运行态字段。
...
6. 把 mode/delegate/multi-agent/workflow 定义成 session capability model 的第一批字段。
7. Web UI 使用结构化 API 切换能力；slash command 只作为人类快捷入口保留。
