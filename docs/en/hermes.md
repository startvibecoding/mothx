...
> **Note:** `context_pressure_threshold` and `budget_pressure_threshold` use a 0-1 scale (0.55 = 55%), not 0-100. The internal comparison automatically converts to percentage. The project template generated by `InitHermesConfig` now includes these defaults explicitly, so they work correctly even without manual configuration.
